...
class VAE(nn.Module):
    def __init__(self, img_size=28**2, h_dim=1600, z_dim=400):
        super(VAE, self).__init__()
        self.z_dim = z_dim
        self.enc = nn.Sequential(
            nn.Linear(img_size, h_dim), nn.ReLU(), nn.Linear(h_dim, z_dim * 2)
        )
        self.dec = nn.Sequential(
            nn.Linear(z_dim, h_dim), nn.ReLU(), nn.Linear(h_dim, img_size)
        )
...
class ConvVAE(nn.Module):
    def __init__(self, img_size=(28, 28), c_num=1, h_dim=3000, z_dim=400):
        ...
        self.enc_conv = nn.Sequential(
            nn.Sequential(
                nn.Conv2d(c_num, 32, 3, padding=1), nn.ReLU(), nn.InstanceNorm2d(32)
            ),
            ResBlock(32),
            ResBlock(32),
            nn.MaxPool2d(2),
            nn.Sequential(
                nn.Conv2d(32, 64, 3, padding=1), nn.ReLU(), nn.InstanceNorm2d(64)
            ),
            ResBlock(64),
            ResBlock(64),
            nn.MaxPool2d(2),
        ...
        self.enc_linear = nn.Sequential(
            nn.Sequential(
                nn.Dropout(), nn.Linear((self.img_h**2) * 64 // 16, h_dim), nn.ReLU()
            ),
            nn.Sequential(
                nn.Dropout(),
        ...
        self.dec_deconv = nn.Sequential(
            ...
            ResBlock(64),
            nn.Sequential(
                nn.ConvTranspose2d(64, 32, 4, 2, 1), nn.ReLU(), nn.InstanceNorm2d(32)
            ),
            ResBlock(32),
            ResBlock(32),
            nn.Sequential(
                nn.ConvTranspose2d(32, 16, 4, 2, 1), nn.ReLU(), nn.InstanceNorm2d(16)
            ),
            nn.Sequential(
