...
for epoch in range(num_epochs):
    for i, (x, _) in enumerate(data_loader):
        ...
        # Scale the KL divergence by epoch number to ease into the KL penalty
    ...
    # Saving model checkpoint every 10 epochs
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), os.path.join(sample_dir, 'svhn_convend_epoch_{}.pth'.format(epoch + 1)))

# End time for the entire training
end_time = time.time()
