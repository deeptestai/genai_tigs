...
weights_path = "./sd/mnist_sd/Mnist_Lora_sdv1.5.safetensors"
...
def run_diffusion_tig(gen_num, pop_size, best_left, perturbation_size, initial_perturbation_size,
    ...
    for n in range(imgs_to_samp):
        ...
        expected_label = int(re.search(r"Number(\d+)", prompt).group(1))
        print("expected label of each number",expected_label)
        with torch.inference_mode():
            init_img = pipe(prompt=prompt, guidance_scale=3.5,
                            num_inference_steps=15, latents=original_lv)["images"][0]
        ...
        if best_image_tensor is not None:
            ...
            with torch.inference_mode():
                perturbed_logit = classifier(input_tensor).squeeze().cpu().numpy()
            predicted_best_label = np.argmax(perturbed_logit)
            print("Predicted label", predicted_best_label)
            # Convert perturbed image to PIL
            pert_img_np = best_image_tensor.squeeze().cpu().numpy() * 255
            perturbed_image_pil = Image.fromarray(pert_img_np.astype(np.uint8))
