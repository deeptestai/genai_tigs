...
from vae.cifar10_vae.vae_model import ConvVAE
import zipfile
...
vae.load_state_dict(torch.load("./vae/cifar10_vae/weights/cifar10_convend.pth", map_location=device, weights_only=True))
vae.eval()
...
def run_vae_tig2(gen_num, pop_size, best_left, perturbation_size, initial_perturbation_size, imgs_to_samp,classifier_choice,classifier_file):
    ...
    result_dir = "./result23_vae_cifar10"
    os.makedirs(result_dir, exist_ok=True)
    original_images_dir = os.path.join(result_dir, "original_images")
    os.makedirs(original_images_dir, exist_ok=True)

    #if image_size_selector is None:
      # print("WARNING: image_size_selector is None. Defaulting to 32.")
      # image_size_selector = 32
   # desired_size = (int(image_size_selector), int(image_size_selector))
    ...
    for img_idx in trange(imgs_to_samp):
        ...
        init_pop = [original_lv + initial_perturbation_size * torch.randn(1, 1024).to(device) for _ in range(pop_size)]
        now_pop = init_pop
        prev_best = np.inf
