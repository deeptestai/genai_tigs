...
from cdcgan.svhn_cdcgan.cdcgan_svhn_train import Generator
...
G.load_state_dict(torch.load("./cdcgan/svhn_cdcgan/weights/netG_epoch_99_ganbsvh.pth", map_location=device))
#classifier.load_state_dict(torch.load(".models/sa/svhn_classifier/model/SVHN_vggnet.pth", map_location=device, weights_only=True))
G.eval()
#classifier.eval()
...
def run_gan_tig1(gen_num, pop_size, best_left,perturbation_size,initial_perturbation_size, imgs_to_samp, classifier_choice,classifier_file):
    ...
    result_dir = "./result23_cdcgan_svhn"
   ...
   # initial_perturbation_size = 0.00177406988143921
    ...
    for img_idx in trange(imgs_to_samp):
        ...
        # Save using your save_image() function
        #save_image(save_tensor,filename=f"original_image_{img_idx}_X{original_label}.png")
        # After saving
        #pil_image_for_gradio = transforms.ToPILImage()(save_tensor.cpu())
       # mod_image = pil_image_for_gradio.resize((image_size_selector, image_size_selector))
