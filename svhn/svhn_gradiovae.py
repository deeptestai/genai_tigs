...
vae.load_state_dict(torch.load("./vae/svhn_vae/svhn_convend.pth", map_location=device, weights_only=True))
vae.eval()
...
def run_vae_tig1(gen_num, pop_size, best_left, perturbation_size, initial_perturbation_size, imgs_to_samp,classifier_choice,classifier_file):
    ...
    for img_idx in trange(imgs_to_samp):
       ...
       # total_images += 1
        ...
        init_pop = [original_lv + initial_perturbation_size * torch.randn(1, 800).to(device) for _ in range(pop_size)]
        now_pop = init_pop
        prev_best = np.inf
