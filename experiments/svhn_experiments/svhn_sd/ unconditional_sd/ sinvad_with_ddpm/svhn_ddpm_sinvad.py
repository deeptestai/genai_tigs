...
all_img_lst = []
num_samples = 100
proj_path = "./evolution_ddpm_svhn/"+proj_name+"_"
os.makedirs(proj_path, exist_ok=True)
os.makedirs(proj_path+'/Newresultjump', exist_ok=True)

print('Creating init image')
scheduler = DDPMScheduler.from_pretrained("Maryamm/ddpm_finetune_svhn")
model = DDPMPipeline.from_pretrained("Maryamm/ddpm_finetune_svhn").to("cuda")
...
for n in range(num_samples):
    ...
    init_pop = [
        original_lv + init_perturbation * torch.randn((3, 32, 32), device=device)
   ...
   # prev_best = np.inf
    for i in range(gen_steps):
        indivs_lv = torch.cat(now_pop, dim=0).view(-1, 3, 32, 32)
        ...
        for k_idx in range(pop_size - best_left):
            ...
            spl_idx = np.random.choice(3, size=1)[0]
            k_gene = torch.cat(
        ...
        prev_best = now_best
    mod_best = parent_pop[-1].view(1, 3, 32,32)
    for t in scheduler.timesteps:
       with torch.no_grad():
