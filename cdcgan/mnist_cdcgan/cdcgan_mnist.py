# https://raw.githubusercontent.com/pytorch/examples/master/dcgan/main.py, layers are changed by using provided link.
...
class_num = 10  # Number of classes in the MNIST dataset


# Generator
class Generator(nn.Module):
    def __init__(self, ngpu, nc=1, nz=100):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(self.nz + 10, 256, 7, 1, 0, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(True),
            # state size: (256, 7, 7)
            ...
            nn.ConvTranspose2d(
                128, 1, 4, 2, 1, bias=False
            ),  # changed 128, 100 to 128, 1
            nn.Tanh()
            # state size: (1, 28, 28)
        )

    def forward(self, x, labels):
        c = self.label_emb(labels)  # This should be [64, 10]
        z = x.view(x.size(0), -1)  # This should be [64, 100]
        x = torch.cat([z, c], 1).view(
            -1, self.nz + 10, 1, 1
        )  # This should be [64, 110, 1, 1]
...
class Discriminator(nn.Module):
    def __init__(self, ngpu, nc=1):
        super(Discriminator, self).__init__()
        self.ngpu = ngpu

        # Embedding for the labels:
        # We'll transform each label to a 1x28x28 tensor
        # (1 channel, spatial dimensions matching the image)
        self.embed = nn.Embedding(10, 28 * 28)

        self.main = nn.Sequential(
            # input is (1 + label embedding) x 28 x 28
    ...
    def forward(self, x, labels):
        ...
        y = self.embed(labels).view(x.size(0), 1, 28, 28)
        x = torch.cat([x, y], 1)
        if x.is_cuda and self.ngpu > 1:
            output = nn.parallel.data_parallel(self.main, x, range(self.ngpu))
        else:
...
if __name__ == "__main__":
    ...
    for epoch in range(opt.niter):
        ...
        # do checkpointing
        torch.save(netG.state_dict(), "%s/netG_epoch_%dcdcgantime.pth" % (opt.outf, epoch))
        torch.save(netD.state_dict(), "%s/netD_epoch_%dcdcgantime.pth" % (opt.outf, epoch))
